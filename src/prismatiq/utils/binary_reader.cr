module PrismatIQ
  module BinaryReader
    ...
    def read_u16_le(slice : Slice(UInt8), idx : Int) : UInt16
      raise IO::Error.new("read_u16_le: out of bounds at index #{idx} (slice size #{slice.size})") if idx + 1 >= slice.size
      slice[idx].to_u16 | (slice[idx + 1].to_u16 << 8)
    end

    def read_u32_le(slice : Slice(UInt8), idx : Int) : UInt32
      raise IO::Error.new("read_u32_le: out of bounds at index #{idx} (slice size #{slice.size})") if idx + 3 >= slice.size
      slice[idx].to_u32 |
